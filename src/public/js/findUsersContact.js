function callFindUsers(element){
    if(element.which === 13 || element.type === "click"){
        ...
        if(!keyword.length){
            alertify.notify("Bạn chưa nhập nội dung để tìm kiếm", "error", 7);
            return false;
        };

        if(!regexKeyword.test(keyword)){
            alertify.notify("Lỗi từ khóa tìm kiếm, chỉ cho phép chữ cái và số, cho phép khoảng trống.", "error",7);
            return false;
        };
